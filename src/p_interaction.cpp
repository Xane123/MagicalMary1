...
int P_DamageMobj (AActor *target, AActor *inflictor, AActor *source, int damage, FName mod, int flags, angle_t angle)
{
	...
	if (!(flags & DMG_FORCED))	// DMG_FORCED skips all special damage checks, TELEFRAG_DAMAGE may not be reduced at all
	{
		...
		if ((target->flags7 & MF7_LAXTELEFRAGDMG)) // TELEFRAG_DAMAGE may only be reduced with LAXTELEFRAGDMG or it may not guarantee its effect.
		{
			if (player && damage > 1)
			{
		...
		target->IsTeammate (source))
	{
		...
		if ((target->flags7 & MF7_LAXTELEFRAGDMG)) 
	...
	if (player)
	{
		...
		if (!(flags & DMG_FORCED))
		{
			...
			if (!(flags & DMG_NO_ARMOR) && player->mo->Inventory != NULL)
			{
				...
				if ((player->mo->flags7 & MF7_LAXTELEFRAGDMG)) //rawdamage is never modified.
				{
		...
		if (player->health <= 0)
		{
			...
			if (!(flags & DMG_FORCED) && (((player->cheats & CF_BUDDHA2) || (((player->cheats & CF_BUDDHA) || (player->mo->flags7 & MF7_BUDDHA)))) && (player->playerstate != PST_DEAD)))
			{
