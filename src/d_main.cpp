	...
	if (vid_fps)
	{
		...
		if ((signed)howlong >= 0)
		{
			...
			for (int i = 2; i < 16; i += 2)
				Dim(twod, 50, 0.15, 0, -6 * textScale, twod->GetWidth(), (ConsoleEntryFont->GetHeight() + i) * textScale);	// [XANE] Draw the background color.
...
void I_UpdateWindowTitle()
{
	...
	case 1:
		...
		{
			titlestr.Format("%s - %s (Level %d, Part %d)", GameStartupInfo.Name.GetChars(), level.LevelName.GetChars(), level.levelnum, level.partnum);
			break;
		}
		[[fallthrough]];
	case 2:
