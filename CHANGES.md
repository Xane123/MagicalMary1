...
* Skyboxes no longer render on mobile/low-end devices (see below for the low-end mod) or on computers if ***mma_lowend.pk3*** is loaded. This was done because even slopes can cause lag on mobile just by looking in their direction if a skybox using them is visible, such as the distant volcano in Hell City (1-1).
